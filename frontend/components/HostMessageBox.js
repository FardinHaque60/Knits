...
function MessageBox({ navigation, id, name, message, picture, date, time }) { //joinStatus = joinable?, state = host/join?
  ...
  const deleteHangout= () => {
    axios.get("http://localhost:8080/api/delete-hangout", {params: {id: id}}) 
      .then(response => {
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.buttonContainer}> 
        <TouchableOpacity style={styles.deleteHangout} onPress={deleteHangout}>
          <Text style={styles.joinHangoutText}> Delete </Text>
        </TouchableOpacity>
